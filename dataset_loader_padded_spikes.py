...
dataset_directory = "/its/home/ts468/PhD/SGSC_Dataset/data/"
...
def SGSC_Loader(dir, 
    ...
    for trial in trange(x_train.shape[0]):
        x_train[trial, 
                x_train_spike_times[trial]["neuron"], 
                x_train_spike_times[trial]["time"]] = 1

    print("loading testing")
    for trial in trange(x_test.shape[0]):
        x_test[trial, 
               x_test_spike_times[trial]["neuron"], 
               x_test_spike_times[trial]["time"]] = 1

    print("loading validation")
    for trial in trange(x_validation.shape[0]):
        x_validation[trial, 
                     x_validation_spike_times[trial]["neuron"], 
                     x_validation_spike_times[trial]["time"]] = 1
